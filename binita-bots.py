...
USER_SESSION_STRING = (
    "BQIvC-cAU5tG5CFEoAIWZn1Dqx8kRIVF2g9g5n6Spcx5pStkJ9mXDU-RC5sYSnyyoKgAmir4h1XKEo1iFNh72SlqeZi5wsMjuGVicRITtN9R_cG4MATMjO8tqkv9WbKs3JdPqtfPlb4cGTwrL6q02pamYQjM3GgQdO3qM-P0_TZAowANtuuII2ZrdsDU3y0oyLo2aXRsadPPo9iMUpQweZsHTQvRDsVF2Z167czoCHdxRD7XeQDkm5421c6kAs24XljPzTEAIRFOoSnMoCtKfUsDJycekgUHJqKS3uynrSqKEcefQle5HXReqjcvfM7iIzKUeTMC9Px_klIA-M_KDQeawwjZPQAAAAHuWj7QAA"
)
...
user_client = Client(
    "user_session_db",  # 👈 File-based session so peers can be cached!
    api_id=API_ID,
    api_hash=API_HASH,
...
async def main():
    ...
    # Stop clients on exit gracefully
    logger.info("Stopping clients...")
    try:
        await app.stop()
    except Exception as e:
        logger.debug(f"App stop error: {e}")
        
    try:
        await user_client.disconnect()
    except Exception as e:
        logger.debug(f"User client stop error: {e}")
...
if __name__ == "__main__":
    try:
        ...
        if "attached to a different loop" in str(e):
            logger.info("Bot gracefully stopped (ignored asyncio loop conflict).")
        else:
            raise
    except Exception as e:
